    ...
    <div className="page-enter max-w-2xl">
      ...
      {editing && (
        <div className="card p-5 mb-5 animate-[fadeIn_0.2s_ease-out]">
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            <div>
              <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Icon</label>
              <input
                value={form.icon}
      ...
      <div className="card overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {cats.map(cat => (
              <tr key={cat.id} className="border-b border-[var(--border)] hover:bg-[var(--bg-secondary)] transition-colors">
                <td className="py-3 px-4 text-xl">{cat.icon}</td>
                <td className="py-3 px-4 font-medium text-[var(--text)]">{cat.name}</td>
                <td className="py-3 px-4 font-mono text-xs text-[var(--text-secondary)]">{cat.slug}</td>
                <td className="py-3 px-4 text-[var(--text-secondary)]">{cat.count}</td>
                <td className="py-3 px-4">
                  <div className="flex gap-2">
                    <button onClick={() => openEdit(cat)} className="p-1.5 rounded-lg hover:bg-accent/10 text-accent">
                      <Pencil size={14} />
                    </button>
                    <button onClick={() => del(cat.id)} className="p-1.5 rounded-lg hover:bg-red-500/10 text-red-500">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </td>
