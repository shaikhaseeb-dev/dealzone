...
const EMPTY_PRODUCT = {
  title: '', shortTitle: '', originalPrice: '', category: 'electronics',
  description: '', features: ['', '', '', '', ''],
  images: [''],
  affiliateLinks: [
    { platform: 'Amazon',   price: '', url: '', label: 'Buy on Amazon',   inStock: true },
    { platform: 'Flipkart', price: '', url: '', label: 'Buy on Flipkart', inStock: true },
  ],
  isTrending: false, featured: false, inStock: true, isNew: false,
  delivery: 'Free delivery in 2-3 days', warranty: '1 Year Warranty',
};
...
export default function AdminProductsPage() {
  ...
  const openNew  = () => { setEditing({ ...EMPTY_PRODUCT, affiliateLinks: EMPTY_PRODUCT.affiliateLinks.map(l => ({ ...l })), features: [...EMPTY_PRODUCT.features] }); setIsNew(true); };
  const openEdit = (p) => { setEditing({ ...p, features: p.features?.length ? [...p.features] : ['','','','',''], affiliateLinks: p.affiliateLinks?.map(l => ({ ...l })) || [] }); setIsNew(false); };
  ...
  const addLink = () => {
    ...
    setField('affiliateLinks', [
      ...
      { platform: next, price: '', url: '', label: `Buy on ${next}`, inStock: true },
    ]);
  };

  const removeLink = (i) => {
    setField('affiliateLinks', editing.affiliateLinks.filter((_, idx) => idx !== i));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const discount    = lowestPrice && origPrice
        ? Math.round(((origPrice - lowestPrice) / origPrice) * 100) : 0;
    ...
    <div className="page-enter">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-[var(--text-secondary)]">{products.length} total products</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="border-b border-[var(--border)] bg-[var(--bg-secondary)]">
              <tr>
                {['Product', 'Category', 'Best Price', 'Platforms', 'Discount', 'Trending', 'Featured', 'Clicks', 'Actions'].map(h => (
            ...
            <tbody>
              {filtered.map(product => (
                <tr key={product.id}
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-3">
                      <div className="relative w-10 h-10 rounded-lg overflow-hidden bg-[var(--bg-secondary)] flex-shrink-0">
                        <Image src={product.images?.[0] || ''} alt={product.shortTitle}
                  ...
                  <td className="py-3 px-4 capitalize text-[var(--text-secondary)] text-xs">{product.category}</td>
                  <td className="py-3 px-4 font-semibold text-[var(--text)]">{formatPrice(product.price)}</td>
                  <td className="py-3 px-4">
                    <div className="flex gap-1 flex-wrap">
                      {product.affiliateLinks?.filter(l => l.inStock !== false).map(l => (
                        <span key={l.platform} className="text-xs px-1.5 py-0.5 rounded bg-accent/10 text-accent font-medium">
                  ...
                  <td className="py-3 px-4 text-[var(--text-secondary)]">{formatNumber(product.clicks)}</td>
                  <td className="py-3 px-4">
          ...
          {filtered.length === 0 && (
            <div className="py-12 text-center text-[var(--text-secondary)]">No products found</div>
          )}
        </div>
      </div>
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto
          bg-black/60 backdrop-blur-sm p-4 pt-6">
          <div className="relative w-full max-w-2xl bg-[var(--bg-card)] rounded-2xl
            border border-[var(--border)] shadow-2xl mb-8 animate-[slideUp_0.25s_ease-out]">
            ...
            <div className="p-5 space-y-5 overflow-y-auto max-h-[75vh]">
              ...
              <fieldset className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input value={editing.shortTitle}
              ...
              <fieldset>
                    ...
                    <div key={i}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <label className="flex items-center gap-1.5 text-xs text-[var(--text-secondary)]
                          ...
                          <input type="checkbox" checked={link.inStock !== false}
                            onChange={e => setLink(i, 'inStock', e.target.checked)}
                ...
                <p className="text-xs text-[var(--text-muted)] mt-2">
                  The lowest in-stock price becomes the displayed &quot;From ₹X&quot; price automatically.
                </p>
              </fieldset>
              ...
              <fieldset>
                ...
                {editing.images?.[0] && (
                  <div className="mt-2 relative w-16 h-16 rounded-xl overflow-hidden border border-[var(--border)]">
                    <Image src={editing.images[0]} alt="preview" fill className="object-cover"
                      onError={() => {}} />
                  </div>
                )}
              </fieldset>
              ...
              <fieldset>
                ...
                <textarea value={editing.description}
                  ...
                  rows={3} placeholder="Short product description (1-2 sentences)…"
            ...
            <div className="flex items-center justify-end gap-3 p-5 border-t border-[var(--border)]">
              <button onClick={close} className="btn-secondary text-sm px-5 py-2.5">Cancel</button>
      ...
      <style jsx global>{`
        .label { display: block; font-size: 0.7rem; font-weight: 600;
          color: var(--text-secondary); margin-bottom: 0.375rem; }
        .input-field { width: 100%; padding: 0.625rem 0.75rem; border-radius: 0.75rem;
