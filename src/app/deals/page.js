...
import { products, deals } from '@/lib/mockData';
...
export default function DealsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (activeTag === 'under-500') return products.filter(p => p.price <= 500);
    if (activeTag === 'best-seller') return products.filter(p => p.tags?.includes('best-seller'));
    return products;
  }, [activeTag]);
  ...
  return (
    <>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 page-enter">
        ...
        <div className="mb-8">
          <h1 className="text-3xl sm:text-4xl font-bold text-[var(--text)] mb-2">All Deals</h1>
          <p className="text-[var(--text-secondary)]">Handpicked discounts updated daily</p>
        </div>
        ...
        <ProductGrid products={filtered} emptyMessage="No deals match this filter right now." />
